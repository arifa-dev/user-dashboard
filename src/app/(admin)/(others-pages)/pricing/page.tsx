...
import { useFetchPlans, PlanWithFeatures } from "@/hooks/useFetchPlans";

// Hardcoded Developer plan
const developerPlan = {
  id: 1,
  name: "Developer",
...
const PlanCard = ({ plan }: { plan: any }) => (
  <div
    className={`rounded-xl bg-white p-6 dark:bg-gray-900 ${plan.borderColor} ${plan.shadow}`}
  >
    ...
    {plan.buttonText && (
      <Link
        href={`/checkout?plan_id=${plan.id}&plan_name=${plan.name}&price=${plan.price}`}
      >
        <button
          className={`mb-6 w-full rounded-lg px-4 py-3 font-medium text-white transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 ${plan.buttonColor}`}
        >
          {plan.buttonText}
        </button>
      </Link>
...
export default function PricingPage() {
  ...
  // Transform backend plans to match UI
  const transformedPlans = plans.map((p: PlanWithFeatures) => ({
    id: p.plan.id,
    name: p.plan.name,
    description: "", // customize if needed
    price: p.plan.price.toLocaleString(),
    period: `/Month`,
    isCurrentPlan: false,
    buttonText: "Subscribe",
    borderColor: "border-2 border-blue-500 dark:border-blue-600",
    buttonColor:
      "bg-blue-500 hover:bg-blue-600 dark:bg-blue-600 dark:hover:bg-blue-700 focus:ring-blue-500",
    shadow: "shadow-lg",
    features: p.features.map((f) => f.feature),
  }));

  const allPlans = [developerPlan, ...transformedPlans];
  ...
  return (
    <div>
      ...
      <div className="min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/[0.03] xl:px-10 xl:py-12">
        ...
        {loading && <p className="text-gray-400 text-center">Loading plans...</p>}
