...
import { calculateTotalOverview } from "@/utils";
import { formatNumber } from "@/utils";

const ReactApexChart = dynamic(() => import("react-apexcharts"), { ssr: false });

export default function NotificationsOverview({ data }: { data: any }) {
  if (!data) {
    return (
  ...
  const overview = calculateTotalOverview(data);
  const series = [Math.abs(overview)];
      ...
      radialBar: {
        ...
        dataLabels: {
          ...
          value: {
            ...
            offsetY: -40,
            color: "#1D2939",
            formatter: (val) => `${overview >= 0 ? "+" : "-"}${Math.abs(val)}%`,
          },
        },
      },
    },
      ...
      <div className="px-5 pt-5 bg-white shadow-default rounded-2xl pb-11 dark:bg-gray-900 sm:px-6 sm:pt-6">
        ...
        <div className="flex justify-between">
          <div>
            ...
            <p className="mt-1 font-normal text-gray-500 text-theme-sm dark:text-gray-400">
              Summary of alerts and activity from your application
            </p>
          </div>
          ...
          <div className="relative inline-block">
            ...
            <Dropdown isOpen={isOpen} onClose={() => setIsOpen(false)} className="w-40 p-2">
              <DropdownItem onItemClick={() => setIsOpen(false)} tag="a">
                View More
              </DropdownItem>
              <DropdownItem onItemClick={() => setIsOpen(false)} tag="a">
                Delete
        ...
        <div className="relative">
          ...
          <span className={`absolute left-1/2 top-full -translate-x-1/2 -translate-y-[95%] rounded-full px-3 py-1 text-xs font-medium
                ...
                {overview >= 0 ? "↑ +" : "↓ -"} {Math.abs(overview)}% growth
          </span>
        ...
        <p className="mx-auto mt-10 w-full max-w-[380px] text-center text-sm text-gray-500 sm:text-base">
          Your notifications {overview >= 0 ? "increased" : "decreased"} this period.
        </p>
      </div>
        ...
        <div className="flex items-center justify-center gap-5 px-6 py-3.5 sm:gap-8 sm:py-5">
          ...
          <div>
            ...
            <p className="flex items-center justify-center gap-1 text-base font-semibold text-red-600 dark:text-red-500 sm:text-lg">
              {formatNumber(Number(data.total_failed))}
          ...
          <div>
            ...
            <p className="flex items-center justify-center gap-1 text-base font-semibold text-green-600 dark:text-green-500 sm:text-lg">
              {formatNumber(Number(data.total_success))}
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div>
            ...
            <p className="flex items-center justify-center gap-1 text-base font-semibold text-indigo-600 dark:text-indigo-400 sm:text-lg">
              {formatNumber(Number(data.total_pending))}
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path fillRule="evenodd" clipRule="evenodd" d="M7.60141 2.33683C7.73885 2.18084 7.9401 2.08243 8.16435 2.08243C8.35773 2.08219 8.54998 2.15535 8.69664 2.30191L12.6968 6.29924C12.9898 6.59203 12.9899 7.0669 12.6971 7.3599C12.4044 7.6529 11.9295 7.65306 11.6365 7.36027L8.91435 4.64004V13.5C8.91435 13.9142 8.57856 14.25 8.16435 14.25C7.75013 14.25 7.41435 13.9142 7.41435 13.5V4.64442L4.69679 7.36025C4.4038 7.65305 3.92893 7.6529 3.63613 7.35992C3.34333 7.06693 3.34348 6.59206 3.63646 6.29926L7.60141 2.33683Z" fill="#465FFF"/>
              </svg>
            </p>
          </div>
